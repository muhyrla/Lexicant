...
async def create_monster():
    ...
    if len(monster_word) < 7 and 'ё' not in monster_word:
        ...
        asyncio.run(create_monster())


async def update_monsters():
...
async def create_particles(x, y):
    for _ in range(20):
                    ...
                    if boss_appeared and monsters[0][2] == 'ponasenkov' and not boss_killed:
                        ...
                        for i, (monster, word, monster_type, angle, base_y) in enumerate(monsters):
                            if word == user_input:
                                ...
                                shot_sound.play()
                                asyncio.run(create_particles(monster.x, monster.y))
                                monsters.pop(i)
                                player_score += 750
                                break
                        user_input = ""
